...
// На Windows используем libpng, на других системах тоже libpng
// spng используется только если явно определено
#ifdef USE_SPNG
// spng код
#else
#define LIBPNG
...
ImageData* _png_load(const char* file){
	...
	image_data = (png_byte*)malloc(row_bytes * t_height);
	if (!image_data) {
		png_destroy_read_struct(&png_ptr, &info_ptr, &end_info);
		fclose(f);
		return nullptr;
...
void png::write_image(std::string filename, const ImageData* image) {
	_png_write(filename.c_str(), image->getWidth(), image->getHeight(), 
		(const ubyte*)image->getData(), image->getFormat() == ImageFormat::rgba8888);
}
